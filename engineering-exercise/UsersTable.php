...
/* retrieve and store keyword from text-field */
$keyword = '';
...
try {
    ...
    /* select attributes where search-keyword is like any of the values */
    $sql = 'SELECT users.user_id, users.email, identification.first, identification.last FROM users JOIN identification ON users.user_id = identification.user_id WHERE users.user_id LIKE :keyword OR users.email LIKE :keyword OR identification.first LIKE :keyword OR identification.last LIKE :keyword';
    $run = $pdo->prepare($sql);
    $data = [
        ':keyword' => '%' . $keyword . '%'
    ];
...
<form name='search' action='' method='get'>
    ...
    Search: <input type='text' name='keyword' value='<?php echo $keyword; ?>' id='keyword'>
    <input type='submit'>
    ...
    if ($page > 1) {
        ...
        /* build a new url retaining query (search-keyword and decremented page-number) */
    ...
    if ($page < $page_count) {
        ...
        /* build a new url retaining query (search-keyword and incrementing page-number) */
        $url = $base_url . "?" . http_build_query($query);
